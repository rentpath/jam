...
exports.completeAuth = function (url, force, callback, check_auth) {
    ...
    if (parsed.auth) {
        ...
        return exports.getAuth(url, callback, check_auth);
    }
...
exports.catchAuthError = function (fn, url, extra_args, callback) {
    fn.apply(null, [url].concat(extra_args).concat(function (err) {
        if (err && err.response && err.response.statusCode === 401) {
            ...
            var opts = extra_args[2];
            var auth = (typeof opts.noauth === "undefined" ? null : opts.noauth);
...
exports.getAuth = function (url, callback, check_auth) {
    var parsed = urlParse( getURL(url) );
